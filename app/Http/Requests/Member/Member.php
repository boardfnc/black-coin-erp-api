...
class Member extends FormRequest
{
    ...
    {
        if($request->route()->named('MemberManagersGet')) {
            return [
                'search_type'                           => 'in:login_id',
        ...
        else if ($request->route()->named('MemberMemberPasswordUpdatePut')) {
            return [
                'password'                              => 'required|string|min:6|max:20',
            ];
        }
        else if ($request->route()->named('MemberMemberStatusUpdatePut')) {
            return [
                'mber_sttus'                            => 'required|in:1,2',
            ];
        }
        else if ($request->route()->named('MemberMemberAccountNumberUpdatePut')) {
            return [
        ...
        else if ($request->route()->named('MemberMemberGradeUpdatePut')) {
            return [
                'mber_grd'                              => 'required|in:1,2,3,4',
            ];
        }
        else if ($request->route()->named('MemberMemberRetrievalPost')) {
            return [
                'mber_id'                               => 'required|numeric',
                'rtrvl_coin'                            => 'required|numeric',
            ];
        }
        else if ($request->route()->named('MemberMemberPaymentPost')) {
            return [
                'mber_id'                               => 'required|numeric',
