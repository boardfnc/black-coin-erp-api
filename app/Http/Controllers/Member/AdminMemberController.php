...
use App\Http\Requests\Member\AdminMember;
...
class AdminMemberController extends Controller
{
     ...
     *         description="성공",
     *         @OA\JsonContent(
     *             @OA\Property(property="data", ref="#/components/schemas/MemberAdminMembersGet_BcMberCollection"),
    ...
    public function index(AdminMember $request)
    {
        $user = auth()->user();
     ...
     *              ),
     *              @OA\Property(
     *                  property="data", ref="#/components/schemas/MemberAdminMemberGet_BcMberResource",
     *              ),
     *              @OA\Property(
     ...
     *
     * @OA\Get(
     *      path="/member/admin-member/dealings/{id}",
     *      summary="회원 거래 정보 조회 - 슈퍼어드민",
     *      tags={"Member - SuperAdmin"},
     ...
     *
     * @OA\Put(
     *      path="/member/admin-member/password-update/{id}",
     *      summary="회원 비밀번호 업데이트 - 슈퍼어드민",
     *      tags={"Member - SuperAdmin"},
     *      description="CA 회원 비밀번호 정보 업데이트 합니다.",
     *
     *      @OA\Parameter(
     *          name="id",
     *          in="path",
     ...
     * )
     */
    public function passwordUpdate(AdminMember $request, $id)
    {
        $user = auth()->user();
     ...
     *
     * @OA\Put(
     *      path="/member/admin-member/status-update/{id}",
     *      summary="회원 상태 업데이트 - 슈퍼어드민",
     *      tags={"Member - SuperAdmin"},
     ...
     * )
     */
    public function statusUpdate(AdminMember $request, $id)
    {
        $user = auth()->user();
     ...
     *
     * @OA\Put(
     *      path="/member/admin-member/account-number-update/{id}",
     *      summary="회원 계좌 정보 업데이트 - 슈퍼어드민",
     *      tags={"Member - SuperAdmin"},
     ...
     * )
     */
    public function accountNumberUpdate(AdminMember $request, $id)
    {
        $user = auth()->user();
     ...
     *
     * @OA\Put(
     *      path="/member/admin-member/grade-update/{id}",
     *      summary="회원 등급 업데이트 - 슈퍼어드민",
     *      tags={"Member - SuperAdmin"},
     ...
     * )
     */
    public function gradeUpdate(AdminMember $request, $id)
    {
        $user = auth()->user();
     ...
     *
     * @OA\Put(
     *      path="/member/admin-member/grade-initialization/{id}",
     *      summary="회원 등급 초기화 - 슈퍼어드민",
     *      tags={"Member - SuperAdmin"},
     *      description="회원 등급 초기화 합니다.",
     *
     *      @OA\Parameter(
     *          name="id",
     *          in="path",
     ...
     * )
     */
    public function gradeInitialization(AdminMember $request, $id)
    {
        $user = auth()->user();
        ...
        $bc_grd_comput_stdr = BcGrdComputStdr::find($bc_mber->mngr_id);

        $mber_grd = '4';

        if($bc_grd_comput_stdr->comput_stdr_se == '1')
        {
            if($bc_mber->rcppay_co >= $bc_grd_comput_stdr->vvip_stdr)
            {
                $mber_grd = '1';
            }
            else if($bc_mber->rcppay_co >= $bc_grd_comput_stdr->vip_stdr)
            {
                $mber_grd = '2';
            }
            else if($bc_mber->rcppay_co >= $bc_grd_comput_stdr->gnrl_stdr)
            {
                $mber_grd = '3';
            }
        }
        else if($bc_grd_comput_stdr->comput_stdr_se == '2')
        {
            if($bc_mber->rcpmny_am >= $bc_grd_comput_stdr->vvip_stdr)
            {
                $mber_grd = '1';
            }
            else if($bc_mber->rcpmny_am >= $bc_grd_comput_stdr->vip_stdr)
            {
                $mber_grd = '2';
            }
            else if($bc_mber->rcpmny_am >= $bc_grd_comput_stdr->gnrl_stdr)
            {
                $mber_grd = '3';
            }
        }
        else if($bc_grd_comput_stdr->comput_stdr_se == '3')
        {
            if($bc_mber->defray_am >= $bc_grd_comput_stdr->vvip_stdr)
            {
                $mber_grd = '1';
            }
            else if($bc_mber->defray_am >= $bc_grd_comput_stdr->vip_stdr)
            {
                $mber_grd = '2';
            }
            else if($bc_mber->defray_am >= $bc_grd_comput_stdr->gnrl_stdr)
            {
                $mber_grd = '3';
            }
        }
        else if($bc_grd_comput_stdr->comput_stdr_se == '4')
        {
            if($bc_mber->tot_delng_am >= $bc_grd_comput_stdr->vvip_stdr)
            {
                $mber_grd = '1';
            }
            else if($bc_mber->tot_delng_am >= $bc_grd_comput_stdr->vip_stdr)
            {
                $mber_grd = '2';
            }
            else if($bc_mber->tot_delng_am >= $bc_grd_comput_stdr->gnrl_stdr)
            {
                $mber_grd = '3';
            }
        }
        ...
        try {
            ...
            $bc_mber->mber_grd = $mber_grd;
            $bc_mber->passiv_grd_at = '2';
            $bc_mber->save();
     ...
     *
     * @OA\Post(
     *     path="/member/admin-member/retrieval",
     *     summary="회원 코인 회수 - 슈퍼어드민",
     *     tags={"Member - SuperAdmin"},
     ...
     * )
     */
    public function retrieval(AdminMember $request)
    {
        $user = auth()->user();
     ...
     *
     * @OA\Post(
     *     path="/member/admin-member/payment",
     *     summary="회원 코인 지급 - 슈퍼어드민",
     *     tags={"Member - SuperAdmin"},
     ...
     * )
     */
    public function payment(AdminMember $request)
    {
        $user = auth()->user();
