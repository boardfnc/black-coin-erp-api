...
use App\Http\Requests\Member\RetrievalMember;
use App\Http\Resources\Dealings\BcMberRtrvlDtlsCollection;
use App\Models\Dealings\BcMberRtrvlDtls;

class RetrievalMemberController extends Controller
{
    /**
     * @OA\Get(
     *     path="/member/retrieval-members",
     ...
     *         description="성공",
     *         @OA\JsonContent(
     *             @OA\Property(property="data", ref="#/components/schemas/MemberRetrievalMembersGet_BcMberRtrvlDtlsCollection"),
    ...
    public function index(RetrievalMember $request)
    {
        $user = auth()->user();
        ...
        $data = BcMberRtrvlDtls::where('mngr_id', $mngr_id)->with(
            'bcMber',
            'bcRirvlMngr'
        );
