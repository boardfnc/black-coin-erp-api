...
use App\Http\Requests\Coin\AdminPurchaseMember;
...
class AdminPurchaseMemberController extends Controller
{
    /**
     * @OA\Get(
     *     path="/coin/admin-purchase-members",
     *     summary="코인 구매(일반) 목록 - 슈퍼어드민",
     *     tags={"Coin - SuperAdmin"},
     *     description="코인 구매(일반)에 대한 정보를 리스트 형태로 받을 수 있습니다.",
     *
     *     @OA\Parameter(
     ...
     *     ),
     *     @OA\Parameter(
     *         name="mumm_today_purchs_co",
     *         in="query",
     *         description="최소당일구매횟수",
     *         @OA\Schema(
     *             type="integer"
     *         ),
     *     ),
     *     @OA\Parameter(
     *         name="mxmm_today_purchs_co",
     *         in="query",
     *         description="최대당일구매횟수",
     *         @OA\Schema(
     *             type="integer"
     *         ),
     *     ),
     ...
     *         description="성공",
     *         @OA\JsonContent(
     *             @OA\Property(property="data", ref="#/components/schemas/CoinAdminPurchaseMembersGet_BcMberDelngDtlsCollection"),
    ...
    public function index(AdminPurchaseMember $request)
    {
        $user = auth()->user();
        ...
        $data = BcMberDelngDtls::where('delng_se', '2')->with(
            'bcMber',
            'bcMngr'
        );
        ...
        if(!empty($request->mumm_today_purchs_co))
        {
            $data->searchBy($request->mumm_today_purchs_co, 'mumm_today_purchs_co');
        }

        if(!empty($request->mxmm_today_purchs_co))
        {
            $data->searchBy($request->mxmm_today_purchs_co, 'mxmm_today_purchs_co');
        }
     ...
     *
     * @OA\Post(
     *     path="/coin/admin-purchase-member",
     *     summary="코인 구매(일반)(코인 지급) - 슈퍼어드민",
     *     tags={"Coin - SuperAdmin"},
     ...
     * )
     */
    public function store(AdminPurchaseMember $request)
    {
        $user = auth()->user();
        ...
        foreach ($request->_bc_mber_delng_dtls as $dtls)
        {
            ...
            $bnus_qy_array[$dtls['mber_delng_dtls_id']] = $dtls['bnus_qy'];
            $compt_qy_array[$dtls['mber_delng_dtls_id']] = $dtls['compt_qy'];
        }
        ...
        foreach ($bc_mber_delng_dtls as $dtl)
        {
            ...
            $sum_qy = $dtl->delng_qy + $bnus_qy_array[$mber_delng_dtls_id];

            if($sum_qy != $compt_qy_array[$mber_delng_dtls_id])
            {
                return response()->json([
                    'status'    => false,
        ...
        foreach ($bc_mber_delng_dtls as $dtl)
        {
            ...
            $bnus_qy = $bnus_qy_array[$mber_delng_dtls_id];
            $compt_qy = $compt_qy_array[$mber_delng_dtls_id];
            ...
            try {
                ...
                $bc_mber_coin_his->coin_se = '2';
                $bc_mber_coin_his->pymnt_coin = $compt_qy;
                $bc_mber_coin_his->coin = $compt_qy;
                ...
                $hold_qy = $bc_mber->hold_coin + $compt_qy;

                $bc_mber->update([
                    'hold_coin' => DB::raw('hold_coin + '.$compt_qy),
                    'rcppay_co' => DB::raw('rcppay_co + 1'),
                    'rcpmny_am' => DB::raw('rcpmny_am + '.$delng_qy),
                    'tot_delng_am' => DB::raw('tot_delng_am + '.$delng_qy)
                ]);
                ...
                $bc_mber_delng_dtls_chghst->delng_sttus = '13';
                $bc_mber_delng_dtls_chghst->hold_qy = $hold_qy;
                $bc_mber_delng_dtls_chghst->delng_qy = $delng_qy;
                $bc_mber_delng_dtls_chghst->bnus_qy = $bnus_qy;
                $bc_mber_delng_dtls_chghst->compt_qy = $compt_qy;
                ...
                $created_de = date('Y-m-d', strtotime($dtl->created_at));
                $count = BcMberDelngDtls::where('delng_se', '2')->where('delng_sttus', '13')->where('created_at', '<=', $created_de.' 00:00:00')->where('created_at', '>=', $created_de.' 23:59:59')->count();

                if($count > 0)
                {
                    $today_purchs_co = $count;
                }
                ...
                $update_bc_mber_delng_dtls->update([
                    ...
                    'bnus_qy' => $bnus_qy,
                    'compt_qy' => $compt_qy,
                    'today_purchs_co' => $today_purchs_co,
                ...
                if(empty($bc_mber_stats))
                {
                    ...
                    $bc_mber_stats->purchs_co = 1;
                    $bc_mber_stats->purchs_qy = $compt_qy ?? 0;
                    ...
                    $bc_mber_stats->purchs_fee_am = $purchs_fee ?? 0;
    ...
    /**
     * @OA\Get(
     *     path="/coin/admin-purchase-member/history",
     *     summary="코인 구매(일반) 상세 이력 - 슈퍼어드민",
     *     tags={"Coin - SuperAdmin"},
     *     description="코인 구매(일반) 상세 이력에 대한 정보를 리스트 형태로 받을 수 있습니다.",
     *
     *     @OA\Parameter(
     *         name="mber_id",
     *         in="query",
     ...
     *         description="성공",
     *         @OA\JsonContent(
     *             @OA\Property(property="data", ref="#/components/schemas/CoinAdminPurchaseMemberHistoryGet_BcMberDelngDtlsChghstCollection"),
     *             @OA\Property(property="status", type="boolean", example=true),
     *             @OA\Property(property="message", type="string", example="success"),
     *             @OA\Property(property="pagination", type="object",
     *                 @OA\Property(property="total", type="integer", example="100"),
    ...
    public function history(AdminPurchaseMember $request)
    {
        $user = auth()->user();
