...
use App\Http\Requests\Coin\AdminSentDetail;
use App\Http\Resources\Dealings\BcMberExchngDtlsCollection;
use App\Models\Dealings\BcMberExchngDtls;

class AdminSentDetailController extends Controller
{
    /**
     * @OA\Get(
     *     path="/coin/admin-sent-details",
     *     summary="보낸 코인 내역 목록 - 슈퍼어드민",
     *     tags={"Coin - SuperAdmin"},
     *     description="보낸 코인 내역 에 대한 정보를 리스트 형태로 받을 수 있습니다.",
     *
     *     @OA\Parameter(
     ...
     *         description="성공",
     *         @OA\JsonContent(
     *             @OA\Property(property="data", ref="#/components/schemas/CoinAdminSentDetailsGet_BcMberExchngDtlsCollection"),
    ...
    public function index(AdminSentDetail $request)
    {
        $user = auth()->user();
        ...
        $data = BcMberExchngDtls::where('exchng_se', '2')->with(
            'bcMber',
            'bcMngr'
        );
