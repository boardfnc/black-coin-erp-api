...
use App\Http\Requests\Coin\AdminReceivedDetail;
use App\Http\Resources\Dealings\BcMberExchngDtlsCollection;
use App\Models\Dealings\BcMberExchngDtls;

class AdminReceivedDetailController extends Controller
{
    /**
     * @OA\Get(
     *     path="/coin/admin-received-details",
     *     summary="받은 코인 내역 목록 - 슈퍼어드민",
     *     tags={"Coin - SuperAdmin"},
     *     description="받은 코인 내역 에 대한 정보를 리스트 형태로 받을 수 있습니다.",
     *
     *     @OA\Parameter(
     ...
     *         name="search_type",
     *         in="query",
     *         description="검색 타입(prtnr_nm:파트너사명, code:코드, login_id:회원 아이디, nm:회원명)",
     *         @OA\Schema(
     *             type="string", enum={"prtnr_nm", "code", "login_id", "nm"}, example="prtnr_nm", default="prtnr_nm"
     *         ),
     *     ),
     *     @OA\Parameter(
     *         name="search_keyword",
     ...
     *         description="성공",
     *         @OA\JsonContent(
     *             @OA\Property(property="data", ref="#/components/schemas/CoinAdminReceivedDetailsGet_BcMberExchngDtlsCollection"),
    ...
    public function index(AdminReceivedDetail $request)
    {
        $user = auth()->user();
        ...
        $data = BcMberExchngDtls::where('exchng_se', '1')->with(
            'bcMber',
            'bcMngr'
        );
        ...
        return new BcMberExchngDtlsCollection($data);
